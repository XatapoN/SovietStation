						...
						alog = "[time2text(world.realtime,"YYYY-MM-DD-(hh-mm-ss)")]:[usr.client.ckey] banned [M.ckey] from [msg]"
						text2file(alog,"data/AdminLog/[usr.client.ckey].txt")
						*/
/datum/admins/Topic(href, href_list)
	...
	/////////////////////////////////////new ban stuff
		...
		notes_add(href_list["warn"], "Warned by [usr.client.ckey]")
		alog = "[time2text(world.realtime,"YYYY-MM-DD-(hh-mm-ss)")]:Warned [href_list["warn"]]"
		text2file(alog,"data/AdminLog/[usr.client.ckey].txt")
		...
		switch(alert("Temporary Ban?",,"Yes","No"))
			...
			if("No")
				...
				reason = input(usr,"Reason?","reason",reason2) as text|null
				if(!reason)	return
		notes_add(Banlist["key"], "[usr.client.ckey] edited [banned_key]'s ban. Reason: [reason]. Duration: [duration]")
		alog = "[time2text(world.realtime,"YYYY-MM-DD-(hh-mm-ss)")]:edited [banned_key]'s ban. Reason: [reason] Duration: [duration]"
		text2file(alog,"data/AdminLog/[usr.client.ckey].txt")
	...
	//JOBBAN'S INNARDS
			...
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				...
				if("No")
					...
					if(reason)
						...
						notes_add(M.ckey, "Banned by [usr.client.ckey] from [msg] - [reason]")
						alog = "[time2text(world.realtime,"YYYY-MM-DD-(hh-mm-ss)")]:banned [M.ckey] from [msg]"
						text2file(alog,"data/AdminLog/[usr.client.ckey].txt")
		...
		if(joblist.len) //at least 1 banned job exists in joblist so we have stuff to unban.
			...
			if(msg)
				...
				notes_add(M.ckey, "Unbanned by [usr.client.ckey] from [msg]")
				alog = "[time2text(world.realtime,"YYYY-MM-DD-(hh-mm-ss)")]:unbanned [M.ckey] from [msg]"
				text2file(alog,"data/AdminLog/[usr.client.ckey].txt")
				href_list["jobban2"] = 1 // lets it fall through and refresh
			return 1
...
*/
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				notes_add(M.ckey, "Banned by [usr.client.ckey].\nReason: [reason]. Duration [mins] minutes.")
				alog = "[time2text(world.realtime,"YYYY-MM-DD-(hh-mm-ss)")]: banned [M.ckey]. Reason: [reason]. Duration [mins] minutes."
				text2file(alog,"data/AdminLog/[usr.client.ckey].txt")
			...
			if("No")
				...
				notes_add(M.ckey, "Banned by [usr.client.ckey].\nReason: [reason].")
				alog = "[time2text(world.realtime,"YYYY-MM-DD-(hh-mm-ss)")]: permabanned [M.ckey]. Reason: [reason]."
				text2file(alog,"data/AdminLog/[usr.client.ckey].txt")
				if(config.banappeals)
					M << "\red To try to resolve this matter head to [config.banappeals]"
				else
