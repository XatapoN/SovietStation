...
#define NUM_OF_POSTER_DESIGNS 19
...
obj/structure/sign/poster/New(var/serial)
	...
	switch(serial_number)
		...
		if(12)
			name += " - Rorschach"
			desc += " End of fear, the 'Rorschach' is here..."
		if(13)
			name += " - GangBang"
			desc += " Come with us!"
		if(14)
		...
		if(15)
			...
			desc += " Into Hell!"
		if(16)
			name += " - Russians with Alliens"
			desc += " Za VDV!"
		if(17)
			name += " - UAC"
			desc += " From Mars with love."
		if(18)
			name += " - NightWatch"
			desc += " I tresnul mir napopolam..."
		if(19)
			name += " - DooM"
			desc += " Demons are our friends."
		else
